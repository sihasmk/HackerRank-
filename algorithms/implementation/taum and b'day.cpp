...
{
    long result;
    bool black_cheaper = false, white_cheaper = false;
    ...
    if (black_cheaper)
        result = (long)b * (long)bc + (long)w * ((long)bc + (long)z);
    else if (white_cheaper)
        result = (long)b * ((long)wc + (long)z) + (long)w * (long)wc;
    else
