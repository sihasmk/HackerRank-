...
{
    int count = 0;
    auto max_iter = max_element(candles.begin(), candles.end());
    int index = distance(candles.begin(), max_iter);

    for (auto it = max_iter; it != candles.end(); it++, index++)
    {
        if (candles[index] == *max_iter)
            count++;
    }
